...
export async function getSnapshot(user: AppUser): Promise<Snapshot> {
  ...
  const [
    ...
    allRows(
      "SELECT m.id,CASE WHEN m.type='INGRESO' AND r.date IS NOT NULL THEN r.date ELSE CAST(m.occurred_at AS TEXT) END occurred_at,m.type,m.document,m.product_id,p.name product,p.code,m.quantity,m.warehouse_id,w.name warehouse,COALESCE(m.notes,'') notes,COALESCE(m.user_email,'') user_email FROM movements m JOIN products p ON p.id=m.product_id JOIN warehouses w ON w.id=m.warehouse_id LEFT JOIN receipts r ON r.number=m.document AND m.type='INGRESO' ORDER BY m.occurred_at DESC,m.id DESC LIMIT 1000",
    ),
    allRows(
